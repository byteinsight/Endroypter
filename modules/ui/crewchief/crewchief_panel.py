...
class CrewChiefPanel(BasePanel):
    ...
    def build(self):
        with dpg.tab(label=self.LABEL, tag=self.root_tag):
            with dpg.group(tag=f"{self.root_tag}_top_section"):
                self.build_button_group()
                self.build_race_controls()
                self.build_pit_deltas()
                self.build_basic_stint()
                self.build_advanced_stint()
    ...
    def build_race_controls(self):

        with dpg.group(horizontal=False, tag="race_configuration_group"):
            self.get_section_header(label="RACE CONFIGURATION", tag="race_config_header")
            ...
            with dpg.table(header_row=False, resizable=False, policy=dpg.mvTable_SizingStretchProp,
                ...
                # Row 1 - RACE_SETUP_GRID
                with dpg.table_row():

                    # Race Type (combo)
                    self.get_vertical_table_combo_cell(tag="race_mode", label="Race Type", items=["Time", "Laps", "Distance"], width=200)

                    # Race Length (mins)
                    self.get_vertical_table_int_cell(tag="race_length_mins", label="Race Length (mins)",
                                                       default=45, width=self.CELL_WIDTH, readonly=False)

                    # Race Length (laps)
                    self.get_vertical_table_int_cell(tag="race_length_laps", label="Race Length (laps)",
                                                       default=20, width=self.CELL_WIDTH, readonly=False)

                    # Race Dist (km)
                    self.get_vertical_table_float_cell(tag="race_distance", label="Race Dist (km)",
                                                       default=1000.00, width=self.CELL_WIDTH, readonly=False)


                    # Lap Length (km)
                    self.get_vertical_table_float_cell(tag="lap_length", label="Lap Length (km)",
                                                       default=5.00, width=self.CELL_WIDTH, readonly=False)


                    # Total Laps (readonly)
                    self.get_vertical_table_text_cell(tag="total_laps", label="Total Laps",
                                                       default="20", width=self.CELL_WIDTH, readonly=True)

    def build_pit_deltas(self):
        with dpg.group(horizontal=False, tag="pit_deltas_group"):
            self.get_section_header(label="PIT DELTAS", tag="pit_deltas_header")
            ...
            with dpg.table(header_row=False, resizable=False, policy=dpg.mvTable_SizingStretchProp,
                ...
                with dpg.table_row():
                    ...
                    # Pit Inputs - Left Top
                    self.get_vertical_table_float_cell(tag="tank_capacity", label="Tank Capacity (l)", default=103.00,
                                                       width=self.CELL_WIDTH, readonly=False)
                    self.get_vertical_table_float_cell(tag="service_time", label="Service Time (s)", default=42.00,
                                                       width=self.CELL_WIDTH, readonly=False)
                    self.get_vertical_table_float_cell(tag="refuelling_rate", label="Fuel Rate (l/s)", default=0.0,
                                                       width=self.CELL_WIDTH, readonly=True)
                    # Lap Time Inputs - Right Top
                    self.get_vertical_table_float_cell(tag="target_fast_lap", label="Fast Lap (s)", default=119.5,
                                                       width=self.CELL_WIDTH, readonly=False)
                    self.get_vertical_table_float_cell(tag="target_lap", label="Target Lap (s)", default=121.5,
                                                       width=self.CELL_WIDTH, readonly=False)
                    self.get_vertical_table_float_cell(tag="target_fuel_lap", label="Fuel Saving Lap (s)",
                                                       default=123.0, width=self.CELL_WIDTH, readonly=False)

                with dpg.table_row():

                    # Pit Deltas - Left Middle
                    self.get_vertical_table_float_cell(tag="total_pit_time", label="Delta Time (s)", default=62.00,
                                                       width=self.CELL_WIDTH, readonly=False)
                    self.get_vertical_table_float_cell(tag="tyre_change_time", label="Tyre Change (s)", default=21.00,
                                                       width=self.CELL_WIDTH, readonly=False)
                    self.get_vertical_table_float_cell(tag="pit_lane_loss", label="PL Loss (s)", default=0.0,
                                                       width=self.CELL_WIDTH, readonly=True)
                    # Fuel Usage - Right Middle
                    self.get_vertical_table_float_cell(tag="fuel_min", label="Fuel Min", default=3.46,
                                                       width=self.CELL_WIDTH, readonly=False)
                    self.get_vertical_table_float_cell(tag="fuel_avg", label="Fuel Avg", default=3.57,
                                                       width=self.CELL_WIDTH, readonly=False)
                    self.get_vertical_table_float_cell(tag="fuel_max", label="Fuel Max", default=3.67,
                                                       width=self.CELL_WIDTH, readonly=False)

                with dpg.table_row():
                    # Free Stop - Left Bottom
                    self.get_vertical_table_float_cell(tag="tyre_change_litres", label="Free Tyres (~litres)",
                                                       default=0.0, width=self.CELL_WIDTH, readonly=True)
                    self.get_vertical_table_float_cell(tag="tyre_change_laps", label="Free Tyres (~laps)", default=0.0,
                                                       width=self.CELL_WIDTH, readonly=True)
                    self.get_vertical_table_float_cell(tag="lap_margin", label="Lap Margin", default=1.00,
                                                       width=self.CELL_WIDTH, readonly=False)

                    # Lap/Stint - Right Bottom
                    self.get_vertical_table_float_cell(tag="basic_lps_min", label="Laps per Stint (Min)", default=0.0,
                                                       width=self.CELL_WIDTH, readonly=True)
                    self.get_vertical_table_float_cell(tag="basic_lps_avg", label="Laps per Stint (Avg)", default=0.0,
                                                       width=self.CELL_WIDTH, readonly=True)
                    self.get_vertical_table_float_cell(tag="basic_lps_max", label="Laps per Stint (Max)", default=0.0,
                                                       width=self.CELL_WIDTH, readonly=True)

    def build_basic_stint(self):
        with dpg.group(horizontal=False, tag="basic_strategy_group"):
            self.get_section_header(label="BASIC STINT", tag="basic_strategy_header")
            ...
            with dpg.table(header_row=False, resizable=False, policy=dpg.mvTable_SizingStretchProp,
                ...
                with dpg.table_row():
                    ...
                    # Stops - Right Top
                    self.get_vertical_table_float_cell(tag="basic_stops_min", label="Stops (Min)", default=0.0,
                                                       width=self.CELL_WIDTH, readonly=True)
                    self.get_vertical_table_float_cell(tag="basic_stops_avg", label="Stops (Avg)", default=0.0,
                                                       width=self.CELL_WIDTH, readonly=True)
                    self.get_vertical_table_float_cell(tag="basic_stops_max", label="Stops (Max)", default=0.0,
                                                       width=self.CELL_WIDTH, readonly=True)

                    # Last Stint - Right Bottom
                    self.get_vertical_table_float_cell(tag="basic_last_stint_min", label="Last Stint (Min)", default=0.0,
                                                       width=self.CELL_WIDTH, readonly=True)
                    self.get_vertical_table_float_cell(tag="basic_last_stint_avg", label="Last Stint (Avg)", default=0.0,
                                                       width=self.CELL_WIDTH, readonly=True)
                    self.get_vertical_table_float_cell(tag="basic_last_stint_max", label="Last Stint (Max)", default=0.0,
                                                       width=self.CELL_WIDTH, readonly=True)

    def build_advanced_stint(self):
        with dpg.group(horizontal=False, tag="advanced_strategy_group"):
            self.get_section_header(label="ADVANCED STINT", tag="advanced_strategy_header")
            ...
            with dpg.table(header_row=False, resizable=False, policy=dpg.mvTable_SizingStretchProp,
                ...
                with dpg.table_row():
                    ...
                    self.get_vertical_table_float_cell(tag="target_fuel", label="Target Fuel/Lap (l)", default=0.00,
                                                       width=self.CELL_WIDTH, readonly=False)
                    self.get_vertical_table_float_cell(tag="target_pace", label="Target Pace (s)", default=0.00,
                                                       width=self.CELL_WIDTH, readonly=False)
                    self.get_vertical_table_checkbox_cell(tag="include_lap_margin", label="Include Lap Margin",
                                                          default=True, readonly=False)
                    self.get_vertical_table_float_cell(tag="leader_pace", label="Leader Avg Lap (s)", default=119.00,
                                                       width=self.CELL_WIDTH, readonly=False)
                    self.get_vertical_table_float_cell(tag="adjusted_total_laps", label="Adjusted Laps", default=0,
                                                       width=self.CELL_WIDTH, readonly=False)
                    self.get_vertical_table_checkbox_cell(tag="use_adjusted_laps", label="Use Adjusted Laps",
                                                          default=False, readonly=False)

    # -----------------------
    # Table Cell Builders
    # -----------------------
    def get_section_header(self, label, tag):
        dpg.add_text(label, tag=tag)

        # Bind header theme
        dpg.bind_item_theme(tag, self.header_theme)
        dpg.bind_item_font(tag, self.ctx.font_manager.title)

    def get_vertical_table_text_cell(self, tag: str, label: str, default: str = 0, width: int = -1, readonly: bool = False):
        self.tags.append(tag)
        label_tag = f"{tag}_label"

        with dpg.group(horizontal=False):
            dpg.add_text(f"{label}", tag=label_tag)
            dpg.add_input_text(tag=tag, default_value=default, readonly=readonly, width=width)

        dpg.bind_item_font(label_tag, self.ctx.font_manager.input_label)
        dpg.bind_item_theme(label_tag, "input_label_theme")
        if readonly:
            dpg.bind_item_theme(tag, "output_cell_theme")
        else:
            dpg.bind_item_theme(tag, "theme_user_input")


    def get_vertical_table_int_cell(self, tag: str, label: str, default: int = 0, width: int = -1, readonly: bool = False):
        self.tags.append(tag)
        label_tag = f"{tag}_label"

        with dpg.group(horizontal=False):
            dpg.add_text(f"{label}", tag=label_tag)
            dpg.add_input_int(tag=tag, default_value=default, readonly=readonly, width=width)

        dpg.bind_item_theme(label_tag, "input_label_theme")
        dpg.bind_item_theme(tag, "theme_user_input")
        dpg.bind_item_font(label_tag, self.ctx.font_manager.input_label)

    def get_vertical_table_float_cell(self, tag: str, label: str, default: float = 0.00, width: int = -1, readonly: bool = False):
        self.tags.append(tag)
        label_tag = f"{tag}_label"

        with dpg.group(horizontal=False):
            dpg.add_text(f"{label}", tag=label_tag)
            dpg.add_input_float(tag=tag, default_value=default, readonly=readonly, width=width)

        dpg.bind_item_theme(label_tag, "input_label_theme")
        dpg.bind_item_theme(tag, "theme_user_input")
        dpg.bind_item_font(label_tag, self.ctx.font_manager.input_label)

    def get_vertical_table_combo_cell(self, tag: str, label: str, items=None, width: int = -1):
        ...
        with dpg.group(horizontal=False):
            dpg.add_text(f"{label}", tag=label_tag)
            dpg.add_combo(tag=tag, items=items, width=width)

        dpg.bind_item_theme(label_tag, "input_label_theme")
        dpg.bind_item_theme(tag, "theme_user_input")
        dpg.bind_item_font(label_tag, self.ctx.font_manager.input_label)

    def get_vertical_table_checkbox_cell(self, tag: str, label: str, default: bool = False, readonly: bool = False):
        self.tags.append(tag)
        label_tag = f"{tag}_label"
