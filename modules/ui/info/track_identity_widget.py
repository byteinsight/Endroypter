import dearpygui.dearpygui as dpg

from modules.ui.base_widget import BaseWidget

class TrackIdentityWidget(BaseWidget):
    LABEL = "Track Identity"
    TAG = "track_identity_group"

    def __init__(self, ctx, prefix=None, show_header=True):
        super().__init__(ctx, prefix, show_header)
        """
        Initialize the widget and inherit the BaseWidget tagging framework.

        Inherited from BaseWidget:
        - section_tag: root container tag unique to this widget instance.
        - theme_tag: tag used to create and bind the widget's theme.
        - table_tag: default table tag for widgets that render a DearPyGUI table.
        - tags: dictionary for storing element tags used during updates.

        `prefix` optionally namespaces all tags to allow multiple instances of
        the same widget to coexist without collisions.
        """

        self.header_value_key = "TrackName"
        self.ctx.logger.error(f"[PitStopWidget.__init__] header_value_key set to {self.header_value_key}")

        # Derived tags
        self.tags = {
            "TrackName": f"{self.section_tag}_track_name",
            "TrackDisplayName": f"{self.section_tag}_track_display_name",
            "TrackDisplayShortName": f"{self.section_tag}_track_display_short_name",
            "TrackConfigName": f"{self.section_tag}_track_config_name",
            "TrackType": f"{self.section_tag}_track_type",
        }

    # BUILD UI
    def inner_build(self):

        with dpg.group(tag=self.section_tag): # , border=True, autosize_x=True):
            with dpg.table(
                tag=self.table_tag,
                header_row=False,
                resizable=False,
                policy=dpg.mvTable_SizingFixedFit,
                borders_innerH=False,
                borders_innerV=False,
                borders_outerH=True,
                borders_outerV=True
            ):

                for i in range(1, 3):
                    dpg.add_table_column(width_fixed=True)
                dpg.add_table_column(width_stretch=True)

                # Row 1
                with dpg.table_row():
                    self.get_vertical_table_cell(label="Display Name", key="TrackDisplayName")
                    self.get_vertical_table_cell(label="Short Name", key="TrackDisplayShortName")
                    self.get_vertical_table_cell(label="Config", key="TrackConfigName")
                    self.get_vertical_table_cell(label="Type", key="TrackType")

        dpg.bind_item_theme(self.table_tag, self.theme_tag)



